...
impl ConfData {
    pub fn new() ->  Result<ConfData,Box<dyn std::error::Error>>  {
        ...
        let ver = ConfData::read_json::<version::ClientVersion>(VERSION_FILE)?; // load versions
    ...
    // 读取json文件
    fn read_json<T:Serialize + for<'de> Deserialize<'de>>(file:&str) -> Result<T,Box<dyn std::error::Error>> {
